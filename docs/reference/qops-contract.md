...
| `install` | `--unregister-task` | Renders all seven workflows from the templates + config, then registers `pickup-loop`'s scheduled task — named `\qops\<project>\pickup-loop`, commanded from `python:` and the root, **disabled** (ADR-0032). Registering never enables, and never disables a task the owner enabled. `--unregister-task` removes it and renders nothing. A host with no scheduler renders the workflows and says so. | 0 |
