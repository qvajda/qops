...
`schema_check.sql`/`schema_check.db` were removed in #210: the SQL-schema
drift check they configured was one project's domain data in the substrate,
and moved to whichever consumer owns a database, registered through
`doctor_checks:` instead. A config still declaring `schema_check:` is
reported by `doctor` by name, naming `doctor_checks` as the replacement.
...
| `install` | `--unregister-task`, `--no-task` | Renders all seven workflows from the templates + config, then registers `pickup-loop`'s scheduled task (unless `pickup_task: false`) — named `\qops\<project>\pickup-loop`, commanded from `python:` and the root, **disabled** (ADR-0032). Registering never enables, and never disables a task the owner enabled. `--unregister-task` removes it and renders nothing. `--no-task` renders and writes scripts identically but skips registration and says so; `doctor` still reports a missing task. A host with no scheduler renders the workflows and says so. | 0 |
| `doctor` | — | Workflow drift, broken doc citations, skill drift, hook installation, the hot-path cap, a removed config key still declared, consumer-declared `doctor_checks`, the pickup task's drift against what the config renders (its enabled/disabled state is *reported*, never a problem and never changed), the three owner/machine preconditions (branch protection, auto-merge settings, workspace trust — best-effort, and anything short of a confirmed yes counts as a problem, but read **only off a PR**: a runner cannot answer them and no PR can fix them), and the open-issue invariants. | 0/1 |
